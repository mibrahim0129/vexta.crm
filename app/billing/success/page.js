  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        if (!userId) {
          // If they hit success page without a session, send them to login.
          router.replace(`/login?next=${encodeURIComponent("/billing/success")}`);
          return;
        }

        // Poll for up to ~10 seconds for webhook → DB update
        ...
        // If it still hasn't shown up, don't loop forever
        setMsg("Payment received. We’re still syncing your account.");
        setErr(
          "If you keep seeing this, go to Pricing and click Manage Billing, or refresh in a minute."
        );
      } catch (e) {
        if (!alive) return;
        setMsg("Payment received. We’re still syncing your account.");
