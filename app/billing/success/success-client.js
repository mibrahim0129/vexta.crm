  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        // 1) Best effort: force a server-side sync using session_id
        if (sessionId) {
          try {
            setMsg("Syncing with Stripe…");
        ...
        setErr(
          "We’re still syncing your account. Please refresh in a moment. If it keeps happening, go to Dashboard and try again."
        );
      } catch (e) {
        if (!alive) return;
        setMsg("Payment received.");
        setErr(e?.message || "Something went wrong while syncing your account.");
      }
    }
